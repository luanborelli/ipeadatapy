...
def latest_updates() -> DataFrame:
    """Returns the latest time series' updates from Ipeadata, from the most to the less recent updated time series."""
    df_latest_updates = metadata().sort_values(by="LAST UPDATE", ascending=False)[
        ["CODE", "NAME", "LAST UPDATE"]
    ]  # .rename(index=str, columns={"SERCODIGO": "CODE", "SERNOME": "NAME", "SERATUALIZACAO": "LAST UPDATE"})
